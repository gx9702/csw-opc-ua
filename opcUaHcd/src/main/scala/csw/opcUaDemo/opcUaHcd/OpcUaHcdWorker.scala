...
object OpcHcdWorker {
  def props(prefix: String): Props = Props(classOf[OpcHcdWorker], prefix)
 ...
 */
class OpcHcdWorker(override val prefix: String) extends Actor with PrefixedActorLogging {
  import context.dispatcher
  import OpcHcd._
  import OpcHcdWorker._
  ...
  private def tryOpcConnection(): Unit = {
    try {
      val opcClient = new OpcUaHcdClient()
      ...
      context.become(connected(opcClient, "0"))
    } catch {
      case ex: Exception =>
   ...
   */
  def submit(setupConfig: SetupConfig, opcClient: OpcUaHcdClient): Unit = {
