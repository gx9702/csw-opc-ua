...
class Hcd2Worker(override val prefix: String) extends Actor with PrefixedActorLogging {
  ...
  private def waitingForOpcServer: Receive = {
    ...
    case _: Setup         => log.error("Not connected to OPC server")
  ...
  private def tryOpcConnection(): Unit = {
    try {
      ...
      opcClient.subscribe(name, (v: DataValue) => {
      ...
      opcClient.subscribe(s"${name}Pos", (v: DataValue) => {
   ...
   */
  def submit(setupConfig: Setup, opcClient: Hcd2OpcUaClient): Unit = {
